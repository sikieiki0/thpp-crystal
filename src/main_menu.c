...
static void Task_NewGameBirchSpeech_ThisIsAPokemon(u8 taskId)
{
    if (!gPaletteFade.active && !RunTextPrintersAndIsPrinter0Active())
    {
        ...
        StringExpandPlaceholders(gStringVar4, gText_ThisIsABoneka);
...
static void AddBirchSpeechObjects(u8 taskId)
{
    ...
    maySpriteId = CreateTrainerSprite(FacilityClassToPicIndex(FACILITY_CLASS_MARY), 120, 60, 0, &gDecompressionBuffer[TRAINER_PIC_SIZE]);
